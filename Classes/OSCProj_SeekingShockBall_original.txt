...
// 		`log("TriggerVolume:LocationName = "$TriggerVolume(OSCSeekTarget).LocationName);
	...
	/*
	 $TriggerVolume(OSCSeekTarget)
	energy_post
	counterweight

	based on targetType, target nearest energy_post or counterweight
	*/
	//`log("AAA");
   ForEach AllActors(class'TriggerVolume',P)   
	...
	//`log("BBB");
        if(FastTrace(P.Location,Location))
        {
			...
			//`log("seekTargetVolumeType: "$seekTargetVolumeType);
			//`log("P: "$P$"");
			//`log("InStr: "$(InStr(""$P.LocationName$"", seekTargetVolumeType)));
			//localActor=P;
			//`log("TriggerVolume:LocationName = "$TriggerVolume(OSCSeekTarget).LocationName);
			//`log("GetItemName: "$GetItemName(TriggerVolume(localActor).LocationName));
			//`log("GetPName: "$P.LocationName);
			if(InStr(P.LocationName, seekTargetVolumeType)>=0)
			{
			
			//`log("testing: "$P.LocationName);
			//`log("Instr result: "$InStr(P.LocationName, seekTargetVolumeType));
			//`log("seekTargetVolumeType: "$seekTargetVolumeType);
